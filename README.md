...
```bash
git clone <your-repo-url> && cd evalcheck
uv sync    # exact versions from uv.lock, installs the `evalcheck` command
```

`uv sync` reproduces the environment from `uv.lock` and installs evalcheck in editable
mode, so there is no separate install step.

The duplicate check downloads `all-MiniLM-L6-v2` (**about 90 MB**) the first time it runs.
Every run after that is offline. If you only want the fast checks, `--skip-duplicates`
avoids the download entirely.

## Usage

```bash
evalcheck examples/sample_evalset.jsonl
```
...
```bash
evalcheck --json evalset.jsonl > report.json      # machine-readable, for CI
evalcheck --skip-duplicates evalset.jsonl         # no model download
evalcheck --duplicate-threshold 0.92 set.jsonl    # stricter: only very close pairs
evalcheck --min-class-share 0.05 set.jsonl        # tolerate smaller classes
evalcheck --min-class-count 20 set.jsonl          # demand more cases per class
```
...
from evalcheck.checks import DiscriminationCheck
...
The scorer is injected so evalcheck never talks to a provider: it works with any stack,
you define what "correct" means, and the test suite needs no API key.

## Example output

Real output from `evalcheck examples/sample_evalset.jsonl` against the bundled
...
- **It does not replace promptfoo, DeepEval, LangSmith, or OpenAI Evals**, and is not
  trying to. Those run evals; they mostly assume the eval set is sound. evalcheck checks
...
- **It audits the set, not the model.** Nothing here tells you whether your model is good.
  A clean evalcheck report means your eval is capable of measuring something, not that the
...
- **Config file for thresholds.** Five flags is already near the limit of what belongs on
  ...
  `evalcheck.toml` with CLI flags as overrides, rather than adding a flag per check.

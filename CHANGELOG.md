...
**The principle, stated first:** evallint audits the reliability of LLM
evaluations. The README opened with "audits LLM eval datasets for the flaws that
make evaluations silently lie to you", which claimed intent a tool cannot
observe and described the object of measurement too narrowly.

**Three things are now distinguished, in a table, before anything else.**
DATASET quality (properties of the file — potential redundancy, class
distribution, potential leakage, ground-truth ambiguity), EVALUATION quality
(properties of the measurement procedure — model separation, evaluator
reliability, statistical comparison, reproducibility, redundancy-adjusted
coverage) and MODEL quality, which evallint does not measure. The dependency
runs one way and is stated: dataset findings bound evaluation quality, which
bounds what may be concluded about a model.

**New `docs/checks.md`** documents all nine analyses under the same eight
headings — what it measures, why it matters, methodology, assumptions,
limitations, example, false positives, false negatives. Where a false-positive
or false-negative rate has been MEASURED the number is given with its source
(the benchmark's held-out split, or the real-dataset history); where it has not,
that is said rather than guessed. Every quoted output was generated by running
the snippet beside it — nothing is illustrative.

**New "What evallint cannot tell you"**, in the README and expanded in
`docs/checks.md`, grouped by WHY the limit exists: out of scope by design,
requires knowing your intent, the information is not in the data, a stated
methodological limit, or nobody has looked. The reasons differ in kind and
collapsing them would suggest they are all fixable.

### Fixed — factual errors in the documentation
- **The README documented `inverted` as a case class.** The 0.4.0 redesign
  renamed it `non_monotonic` and added `separating` and `indeterminate`; the
  class table was updated then, three prose mentions were not, and two classes
  were never mentioned at all. Names now come from the enum, and a test asserts
  every `CaseClass` value appears in the docs while `inverted` does not.
- **"Only four checks. Ambiguous ground truth is a real problem and are not
  implemented."** Broken grammar, and wrong since 0.7.0 — the check has shipped
  for eight releases. The roadmap still listed it as unbuilt.
- Terminology corrected throughout: "inversion" to non-monotonic, "silent lie"
  to unstated gap, "too easy" to "provides limited evidence for model
  separation".

### Added — tests that keep the documentation honest
17 new, on top of the existing count and flag guards:
- every check in `docs/checks.md` carries all eight required headings;
- the documented case classes match the enum (the check that would have caught
  `inverted`);
- the README states the principle, distinguishes the three qualities, and has a
  "What evallint cannot tell you" section;
- the precise terms are actually used (`potential redundancy`, `potential
  leakage`, `ground-truth ambiguity`, `limited evidence for model separation`,
  `not identifiable`);
- eight unsupportable claims are absent, each of which was present before this
  rewrite, so they cannot come back;
- the precision and recall figures quoted in `docs/checks.md` match
  `benchmarks/results/test.json`. A stale measured number is a false claim about
  performance, which is worse than no number.

808 tests, up from 791. No code behaviour changed in this release.
